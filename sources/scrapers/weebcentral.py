...
from sources.base.scraper import Scraper, to_thread
import json
...
class WeebCentralWebs(Scraper):
  ...
  async def get_chapters(self, data, page: int=1):
    ...
    try:
      ...
      try: chapters = await self.get(new_link, cs=True)
      except: chapters = None
      if chapters:
        bs = BeautifulSoup(chapters, "html.parser")
        chapter_tags = bs.find_all("a", class_=lambda x: x and "hover:bg-base-300" in x)
        chapters_list = []
        for a in chapter_tags:
          try:
            ...
            if episode_title and ch_url:
              chapters_list.append({"title": episode_title, "url": ch_url})
          except:
            continue
    ...
    for ch in data['chapters']:
      try:
        if isinstance(ch, dict):
          ch_url = ch.get('url', '')
          episode_title = ch.get('title', 'Unknown')
        else:
          ch_url = ch.get("href") if hasattr(ch, 'get') else ''
          outer_span = ch.find("span", class_="grow flex items-center gap-2") if hasattr(ch, 'find') else None
          episode_title = None
          if outer_span:
            inner_spans = outer_span.find_all("span", recursive=False)
            if inner_spans:
